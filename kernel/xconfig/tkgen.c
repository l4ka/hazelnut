    ...
    int menu_num, int line_num )
{
	...
	printf(".menu%d.config.f.x%d.x configure -state normal} else { .menu%d.config.f.x%d.x configure -state disabled}\n",
	       menu_num, cfg->cfg_parent->menu_line,
	       menu_num, cfg->cfg_parent->menu_line );
	break;
	 ...
	 */
	{
		...
		if (cfg->depend)
		    printf("\tglobal %s\n\tif { $%s == \"%s\"} then { write_tristate $cfg $autocfg %s 1 $%s } else { write_tristate $cfg $autocfg %s 0 $%s }\n",
			   cfg->depend, cfg->optionname, cfg1->label,
			   cfg1->optionname, cfg->depend,
			   cfg1->optionname, cfg->depend );
		else
