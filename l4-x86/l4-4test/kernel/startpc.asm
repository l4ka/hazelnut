include l4pre.inc
  ...
  Copyright GMD, L4.START.PC, 30,05,96, 46 
...
protected_mode_start:
  ...
  pop   ecx                              ;
  sub   ecx,offset current_eip           ;

  mov   eax,[ebx].mbi_mem_low
  shl   eax,10
  and   eax,-pagesize
  mov   [ecx+dedicated_mem0+physical_kernel_info_page].mem_begin,eax
  mov   [ecx+dedicated_mem0+physical_kernel_info_page].mem_end,MB1

  mov   eax,[ebx].mbi_mem_high
  shl   eax,10
  add   eax,MB1
  and   eax,-pagesize
  mov   [ecx+main_mem+physical_kernel_info_page].mem_begin,0
  mov   [ecx+main_mem+physical_kernel_info_page].mem_end,eax
...
;
;
;       Remark: If L3 kernel is loaded by DOS, INT 13h vector will be
...
init_nmi:
  ...
  in    al,sys_port_b                                           ;Ž90-10-21...
  test  al,(1 SHL paritychk_signal_bit)+(1 SHL iochk_signal_bit)
  jnz   memory_failure                                          ;...Ž

  mov   bl,nmi
  mov   bh,0 SHL 5
  mov   eax,offset memory_failure+PM
...
memory_failure_text    db ' : MEMORY_FAILURE'                   ;Ž90-10-21
...
memory_failure:
  ...
  mov   esi,offset memory_failure_text
  in    al,sys_port_b                                           ;Ž90-10-21...
  shr   al,iochk_signal_bit
  add   al,'0'
  mov   [esi],al                                                ;...Ž
  mov   ecx,sizeof memory_failure_text
  mov   ah,0F0h
  call  direct_console_out
...
init_interrupt_catcher:
  ...
  mov   bl,irq7_intr
  mov   bh,0 SHL 5
  mov   eax,offset lost_interrupt_catcher+PM
  call  define_idt_gate
...
init_rtc_timer:
  ...
  mov   bl,irq8_intr
  mov   bh,0 SHL 5
  mov   eax,offset rtc_timer_int+PM
  call  define_idt_gate
  ...
  inrtc rtc_reg_c           ; reset timer intr line             ;Ž90-06-01
