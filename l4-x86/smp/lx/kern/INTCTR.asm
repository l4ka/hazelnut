include l4pre.inc 
  ...
  Copyright IBM, L4.INTCTR, 07,01,00, 57
...
init_intctr:
  ...
  IFZ eax,1
	...
	;<cd> copy initial idt to idt  (valid as long as idt is shared)
	mov   bl,0
...
define_idt_gate:
  ...
  ;<cd> idt entry = offset idt + 8 * interrupt nr
  movzx edi,bl
  shl   edi,3
  add   edi,offset idt

  ;<cd> define idt entry ebx:eax
  ;<ds> idt gate descriptor:	+4 ofs:31-16 p:15 dpl:14-13 descr type:12-8= 0D110 0/res:7-0
  ;<ds>                         +0 sgm descr: 31-16 ofs:15-0
  shld  ebx,eax,16
  rol   ebx,16
  rol   eax,16
  ...
  ;<cd> store descriptor entry in idt
  mov   [edi],eax
  mov   [edi+4],ebx
...
initial_debug_exception_handler:
  ...
  ;<cd> set debug_exception code and jump to exception
...
exception:
  ...
  ;<cd> exception code in al (mask rest of eax)
  and   eax,000000FFh

  ;<cd> if (segment not present) or (stack exception) or (general protection)
  ...
  IFZ   al,general_protection   ; recall: linear_space is only valid
        ...
        IFB_  ebx,sizeof gdt 
	      ...
	      add edi,eax
	      pop eax
              test  byte ptr ss:[edi*8+task_proot+3-8].switch_ptr,80h 
              CANDNZ 
...
;----------------------------------------------------------------------------
	...
	;<pr> buggy, not jet updated from current version. faulty on 486: racing condition
