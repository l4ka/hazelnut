include l4pre.inc 
  ...
  Copyright IBM+UKA, L4.EMUCTR, 24,08,99, 40030
...
init_emuctr:
  ;<cd> IF bootstrap (IDT shared)
  lno___prc eax
  IFZ eax,1	
	;<cd> load general protection handler to IDT
	mov   bl,general_protection
	mov   bh,0 SHL 5
	mov   eax,offset general_protection_handler	;seems to be reentrant 
	call  define_idt_gate

	;<cd> load invalid opcode handler to IDT
	mov   bl,invalid_opcode
	mov   bh,0 SHL 5
	mov   eax,offset invalid_opcode_handler		;seems to be reentrant 
	call  define_idt_gate
   FI

  ;enable enhanced v86 features if present 
...
signal_virtual_interrupt:
  ...
  ;<cd> load ip bottom address
  lea___ip_bottom ebp,ebp
  ;<cd> set virtual interrupt flag
...
XHEAD virtual_interrupt_pending
  ...
  ;<cd> return if virtual interrupt flag is not set
  test  eax,(1 SHL vif_flag)
  xret  z

  ;<cd> reset virtual interrupt pending flag
  btr   [esp+iv_eflags],vip_flag
  ;<cd> set nmi and jump to exception
  mov   al,nmi
  jmp   exception
