...
init_pagfault:
  ...
  ;<cd> set interrupt vector for page fault handler
...
;----------------------------------------------------------------------------
  ...
  klign 16
	...
	;<pr> concurrent fault on the same address has to be handled
	
page_fault_handler:
  ;<cd> check if there is a real PF

	

	
	
  ;<cd> call pl0 handler if faulting code was pl0
  cmp   ss:[esp+iret_cs+4],phys_mem_exec
  xc    z,page_fault_pl0,long

  ;<cd> ipre present with exception code
  ipre  ec_present

  ;<cd> safe stack pointer
  mov   ebp,esp

  ;<cd> else load interrupted instruction pointer
  mov   ebx,-1
  IFNZ
        mov   ebx,[ebp+ip_eip]
  FI

  ;<cd> load faulting address
  mov   edx,cr2

  ;<cd> check for read or write fault
  ...
  ;<cd> call special pagefault handler for small address space pf
  cmp   edx,offset small_virtual_spaces
  xc    ae,perhaps_small_pf,long
    
  ;<cd> if pagefault within big hw address space
  IFB_  edx,<virtual_space_size>,long
	
	;<cd> if thread is locked running 
	...
	;<cd> pf within ipc 
        ...
        CANDNZ			; else
	      ...
	      ;<cd> setup PF ipc to pager
              mov   esi,[ebp+pager]	; dest = pager id 
              sub   edi,edi		; mw2 = 0  
              sub   ecx,ecx		; timeout snd, rcv, pf = infinity
              sub   eax,eax		; snd dope = register ipc only
              mov   ebp,32*4+map_msg	; rcv dope = entire address space (fpage = 0,32) + map message
					; edx mw0 = faulting address/4,write fault, ~
					; ebx mw1 = user lvl eip
 	      push  0
              push  phys_mem_exec
              push  offset ipcret
              jmp   ipc_sc
	...
	;<cd> PF happend within IPC
	;<cd> store IPC state
        call  push_ipc_state	
        
        IFNZ
	      ;<cd> setup ipc to pager
              mov   esi,[ebp+pager]
              sub   edi,edi
              sub   eax,eax
              push  edx
	      ...
	      ;<cd> if PF ipc did not fail
              test  al,ipc_error_mask
              IFZ
	      ;<cd> and page is present
              test__page_present edx
              CANDNC
		    ;<cd> reload IPC state and continue
                    call  pop_ipc_state

                    ipost
              FI
        FI
	;<cd> otherwise cancel IPC
        jmp   cancel_if_within_ipc

  FI

  ;<cd> PF in upper memory region

  ;<cd> if PF eip != -1 (PF occured in kernel code) => PF Exception
  mov   al,page_fault
  cmp   ebx,-1
  jnz   exception
  ...
  ;<cd> if Faulting address below small space area => addressing error
  cmp   eax,shared_table_base
  jb    short addressing_error

  ;<cd> if Faulting address in shared table area => shared table fault
  cmp   eax,shared_table_base+shared_table_size-1
  jbe   shared_table_fault

  ;<cd> PF before io page base map => addressing error
  cmp   eax,offset iopbm
  jb    short addressing_error

  ;<cd> PF in IO page base map  => iopbm fault
  cmp   eax,offset iopbm+sizeof iopbm-1
  jbe   own_iopbm_fault

  ;<cd> PF before com0 space => addressing error
  cmp   eax,offset com0_space
  jb    short addressing_error

  ;<cd> PF in com1 space => com space write fault
...
XHEAD   occurred_within_ipc
  ...
  ;<cd> set PF EIP to 0 if within short IPC path
  mov   eax,[esp+ip_eip]
...
XHEAD page_fault_pl0
  ...
  mov   eax,ss:[esp+iret_eip+4+4] 
		...
		lno___prc ecx
	        mov   ecx,ss:[kernel_proot+8*ecx-8]
        FI
