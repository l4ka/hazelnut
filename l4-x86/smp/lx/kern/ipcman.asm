include l4pre.inc 
  ...
  Copyright IBM+UKA, L4.IPCMAN, 18,08,99, 9174, K 
...
ipc_sc:
  ...
  ;<cd> receive only if user specifies send buffer beyond his address space
  cmp   eax,virtual_space_size-MB4
  jae   receive_only

  ;<cd> load sender tcb (ebx)
  mov   ebx,esp   
  push  edx
  ...
  ;<cd> load linear kernel space into data segment
  mov	ds,edx
  
  ;<cd> store receive descriptor in tcb
  mov   [ebx+rcv_descriptor],ebp
  mov   ebp,esi  

  ;<cd> store destination threadID (ebp=esi) in tcb (wait for this thread to enter receive state)
  mov   [ebx+waiting_for],esi

	;<cd> IPC redirection
        and   al,(NOT ipc_control_mask)+deceit+map_msg  ;REDIR begin ----------------------------

	;<cd> mask task no of source and destination	;
        ...
        shr   edx,task_no-log2_tasks-2                  ;
							;
	;<cd> lookup ipc redirection table at index: (dest_task_no*4){column}+(src_task_no*tasks*4){row}
        mov   edx,[edx+ebp+redirection_table]           ;

	;<cd> redirect or lock if ipc not transparent   ;
        cmp   edx,ipc_transparent                       ;
        xc    nz,redirect_or_lock_ipc,long              ;
                                                        ;
        mov   ebp,esi                                   ;REDIR ends ----------------------------

  ;<cd> load destination tcb address
  and   ebp,mask thread_no  

  ;<cd> store second message word
  mov   [ebx+mword2],edi  
  add   ebp,offset tcb_space

  ;<cd> load src threadID	
  mov   edi,[ebx+myself]
                                ;REDIR:   and   al,(NOT ipc_control_mask)+deceit+map_msg  
  ;<cd> load dest threadID
  mov	edx,[ebp+myself]
  xor	edi,esi
  ...
  ;<cd> load processor number [MP]
  lno___prc edi
	
  ;<cd> test if destination exists
  cmp   esi,edx
  jnz   ipc_dest_not_existent

  ;<cd> if destination thread is not on current processor [MP]
  cmp edi,[ebp+proc_id]
  jnz ipc_cross_processor	


	
  ;<cd> ipc on local processor

  ;<cd> if deceit bit is set ipc is propagated (propagate pre)
  test  al,deceit
  ;<cd> if propagatee in src task or in dest task receiver gets propagatee id and waits for receive from this thread
  ;<cd> else check for direction preceiving IPC
  xc    nz,propagate_pre,long 	

  ;<cd> get fine state of destination thread
  mov   dl,[ebp+fine_state]
  
  and   dl,nwait+nclos
  ;<cd> IF dest thread is ???
  IFLE                                      ; Greater : nwait=SF=OV=0 and ZF=0
	;<cd> IF dest thread is ???
	IFZ
		;<cd> IF dest thread is in closed wait
		;<cd> does dest thread wait for src thread
		mov   esi,[ebp+waiting_for]
		mov   edi,[ebx+myself]   
		cmp   edi,esi      
	FI
	;<cd> IF (dest thread is in closed wait but not for src thread) OR (???) THEN check for pending or autopropagation
...
receive_message:
  ...
  mov   dh,[ebx+list_state]
  IF____xc ae,send_only,long

  ELSE__

	and   edx,(is_polled SHL 8) + nclos   
              
	cmp   edx,(is_polled SHL 8) + nclos
	jz    fetch_next
...
XHEAD propagate_pre
  ...
  ;<cd> get address propagatee ID parameter on user level stack
  mov   edi,[esp+iret_esp+(3+1)*4]  ; must be read from user space,
  sub   edx,edx

  ;<cd> clear propagatee_tcb field
  mov   [ebx+propagatee_tcb],edx
  ;<cd> load propagatee thread id
  mov   edi,es:[edi]                ; potentially small space

  ;<cd> set task no of sender and receiver to 0 if equal to the task no of propagatee
  mov   edx,ebx
  mov   esi,ebp
  xor   edx,edi
  xor   esi,edi

  ;<cd> store propagatee ID in virtual sender field in the tcb 
  mov   [ebx+virtual_sender],edi

  ;<cd> IF src task equals propagatee's task
  test  edx,mask task_no                          ; propagation if deceited in dest or src task
  CORZ
  ;<cd> COR dest task equals propagatee's task
  test  esi,mask task_no
  IFZ
	;<cd> load propagatee's tcb address
        lea___tcb esi,edi
	
	;<cd> IF tcb writeable
        test__page_writable esi
        IFNC
	;<cd> CAND propagatee tcb valid 
        CANDZ [esi+myself],edi
        mov   dl,[esi+fine_state]
        mov   edi,[esi+waiting_for]
        and   dl,NOT nwake
	;<cd> CAND propagatee is closed waiting
        CANDZ dl,closed_wait
	;<cd> CAND waiting for src thread
        CANDZ [ebx+myself],edi

	      ;<cd> store propagatee's tcb address in src thread
              mov   [ebx+propagatee_tcb],esi
	      ;<cd> set propagatee to wait for src thread
              mov   edi,[ebp+myself]
  ...
  ;<cd> ELSE src and dest task differ from propagatee's task
	;<cd> check for direction preceiving deceit
	
  mov   edx,[ebx+myself]                                ;REDIR begin --------------------------------
  mov   esi,[ebp+myself]                                ;
  xor   esi,edx                                         ;
  xor   edx,edi                                         ;
  or    edx,esi                                         ;
  ...
  IFZ                                                   ; ;       and redir path
        push  ecx                                       ; ;
        push  ebp                                       ; ; note that this implies that I am     
                                                        ; ;       NOT the chief/redirector
        and   ebp,mask task_no                          ; ;
        mov   esi,edi                                   ; ;       see below for that case 
        shr   ebp,task_no-2                             ;
        mov   ecx,16                                    ;
        DO                                              ;
...
XHEAD enter_wakeup_for_receive_timeout
  ...
  mov   dword ptr [esp+4+4],offset receive_timeout_ret
...
XHEAD ipc_long
  ...
  ;;      mov   [ebx+com_partner],ebp
	...
	add   eax, edi
	pop   edi
	mov   eax,[(eax*8)+task_proot-8].proot_ptr
...
XHEAD string_to_com1_space
  ...
  ;<cd> load page directory of task: tcb (ebp)
  lea___pdir ecx,ebp
	
  push  eax
  mov   eax,ds:[ebp+proc_id]
  dec   eax
...
ipc_cross_processor:	
  ...
  ke 'cross processor ipc'

  ; ebp = dest tcb, edi = proc_id, edx = dest_id_tcb, eax = snd descriptor, ebx = src_tcb, ecx = timeouts, esi = dest_id
  ;; setup mailbox
	
  push eax  
  imul edi,(mailbox_size+4)
  imul edx,[ebp+proc_id],((1 SHL max_cpu)*(mailbox_size+4))
  add  edx, offset mailbox
  add  edx, edi
	
  ; ebp = dest tcb, ebx = src tcb, ecx = timeout,  esi = dest_id edi scratch, edx = offset mailbox begin

  sub eax,eax		
  sub edi,edi
  inc edi
  lock cmpxchg ds:[edx],edi	; set bit 0 and read old value (should be 00 for free + unlocked)
				; 01 free but locked, 10 not free but unlocked 11 locked and not free
  IFZ				; mailbox was free
mailbox_locked:	
	; write to mailbox
	pop eax
	mov ds:[edx+4],eax	; snd descriptor 
	pop eax
	pop edi	
	mov ds:[edx+8],ebp	; dest_tcb 
	mov ds:[edx+16],ecx	; timeouts 
	mov ds:[edx+24],edi	; mword 1 
	mov edi,ds:[ebx+mword2]
	mov ds:[edx+12],ebx	; src_tcb 
	mov ds:[edx+20],eax	; mword 0 
	mov ds:[edx+28],edi	; mword 2 
	; mailbox_IPC: snd desriptor / dest_tcb / src_tcb / timeouts / mw0 / mw1 / mw2
	 
	;set sender wait for proxy thread
		; include sender in wait list to be checked within the timer interrupt
	
	; set sender locked running	// only needed for active polling, else locked_waiting
	mov [ebx+fine_state],locked_running
	
	; unlock mailbox
	mov edi,2
	mov ds:[edx],edi		; 10 not free but unlocked
	
	; send ipi


	; poll for proxy thread completion
	mov [ebx+proxy_completion],0
	DO
		mov edx,[ebx+proxy_completion]
		REPEATZ
		; switch to anyone
	OD
	; setup ipc pre: setup receive?, ipc fail, ipc complete
	IFZ dh,0		; send completed correctly
		; setup old stack
	
		jmp receive_message
	FI
	; setup completion message of send part
	mov eax,edx
	mov [ebx+fine_state],running
	ke 'end of message'
	iretd	
...
attach_intr:
  ...
  IF    kernel_x2
        ...
        IFNZ
           ...
           mov ebx,10000h  
              ...
              lea   eax,[(ecx*intr1_intr0)+intr_0_P2]
              lea   ebx,[ecx+irq0_intr]
              mov   bh,0 SHL 5
  ...
  push  eax
  push  ebx
  lea   eax,[(ecx*intr1_intr0)+intr_0]
  lea   ebx,[ecx+irq0_intr]
  mov   bh,0 SHL 5
  call  define_idt_gate
  pop   ebx
...
init_ipcman:
  ...
  ;<cd> define IDT gate for ipc sc and id nearest sc
  mov   bh,3 SHL 5

  mov   bl,ipc
  mov   eax,offset ipc_sc
  call  define_idt_gate

  mov   bl,id_nearest
  mov   eax,offset id_nearest_sc
  call  define_idt_gate
