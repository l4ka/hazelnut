include l4pre.inc 
  ...
  Copyright IBM, L4.PAGFAULT, 04,07,00, 9567, K
...
small_space_com_write_fault:
  ...
  mov   ebp,[ebp+com_partner]               ; com partner is tcb address
  sub   eax,[ebp+as_base]
  call  tunnel_to
...
XHEAD page_fault_pl0
  ...
  IFZ   eax,offset ipcman_sysexit_to_user_instruction+KR
        ...
        mov   [esp+iret_eip+4+4],offset ipc_exit+KR
  ...
  FI
        ...
        CANDZ
              and   ecx,-pagesize
              xptab eax,eax
              and   dh,byte ptr ss:[(eax*4)+ecx+PM]
        FI
        and   dh,dl
        IFZ   dh,dl
              and   byte ptr ss:[esp+iret_eflags+5*4],NOT (1 SHL c_flag)
        ELSE_
              or    byte ptr ss:[esp+iret_eflags+5*4],1 SHL c_flag
        FI
        add   ss:[esp+iret_eip+5*4],4
