include l4pre.inc
  ...
  Copyright IBM+UKA, L4.KDEBUG, 23,04,00, 581
...
kdebug_io_call_tab  dd    kd_outchar+KR  ;  0
                    dd    outstring+KR   ;  1
                    dd    outcstring+KR  ;  2
                    dd    clear_page+KR  ;  3
                    dd    cursor+KR      ;  4
...
default_kdebug_exception:
  ...
  IFZ   al,3
        ...
        inc   ebx
        IFZ   ah,4
        CANDZ <byte ptr ds:[ebx+4]>,<PM SHR 24>
              mov  ebx,[ebx+1]
              add  ebx,4
        FI
...
set_breakpoint:
  ...
  FI
  mov   bl,debug_exception
  mov   eax,offset kdebug_debug_exception_handler+KR
  call  set_exception_handler
  pop   eax
...
display_tcb:
  ...
  IFDEF waddr
        mov   eax,[esi+waddr]
        IFAE  eax,offset small_virtual_spaces
              kd____disp  <6,4,60,'   waddr   : '>
              kd____outhex32
        ELSE_
...
display_regs_and_stack:
  ...
  push  phys_mem
  pop   ds
  test  [physical_kernel_info_page].kdebug_permissions,kdebug_dump_regs_enabled
  pop   ds
  IFZ
        mov   al,0
        ret
...
page_fault_prot:
  ...
  IFZ   al,'*'
        ...
        FI
        mov   eax,offset show_page_fault+KR
        mov   bl,page_fault
        call  set_exception_handler
        ret
  FI
...
show_page_fault:
  ...
  mov   ebx,cr3                             ;     ensures that kdebug can run even if current
  push  ebx                                 ;     current address space has not sufficient
  mov   ebx,ss:[kernel_proot]               ;     memory mapped for kdebug
  mov   cr3,ebx                             ;
  mov   eax,cr2
  and   eax,-pagesize
  IFNZ  eax,<offset ldt>,long
  CANDAE eax,[page_fault_low+PM]
  CANDBE eax,[page_fault_high+PM]
  CANDB  eax,<offset com0_space>                  ; do not protocol pseudo PFs in comspace
.errnz (offset com1_space+com1_space_size)        ; otherwise a periodic inzterrupt (kb, e.g.) leads
                                                  ; to starvation if prot is on
        mov   ebx,cr2
        mov   ecx,[esp+ip_eip+4]
        lno___thread eax,esp
        shl   eax,8
...
ipc_prot:
  ...
  IFZ   al,'*'
        ...
        IFZ   [ipc_prot_handler_active],0
              ...
              mov   [ipc_handler],eax
        FI
        mov   eax,offset show_ipc+KR
        mov   bl,ipc
        call  set_exception_handler
        IFZ   [ipc_sysenter_address],0
              mov   ecx,sysenter_eip_msr
              rdmsr
              mov   [ipc_sysenter_address],eax
              mov   eax,offset show_ipc_sysenter+KR
              sub   edx,edx
              wrmsr
        FI
        ret
  ...
  IFZ   al,'-'
        IFNZ  [ipc_prot_handler_active],0
              ...
              mov   bl,ipc
              call  set_exception_handler
              mov   eax,[ipc_sysenter_address]
              IFNZ  eax,0
                    mov   [ipc_sysenter_address],0
                    mov   ecx,sysenter_eip_msr
                    sub   edx,edx
                    wrmsr
              FI
...
show_ipc_sysenter:
  ...
  jmp   ss:[ipc_sysenter_address+PM]
...
monit_exception:
  ...
  IFZ   al,'+'
        ...
        mov   eax,offset exception_monitor+KR
        call  set_exception_handler
  FI
...
exception_monitor:
  ...
  mov   al,ss:[monitored_exception+PM]
  mov   ebp,esp
  DO
        ...
        CANDB al,15
              ...
              movzx ebx,ss:[monitored_ec_min+PM]
              movzx ecx,ss:[monitored_ec_max+PM]
              IFBE  ebx,ecx
                    cmp   eax,ebx
                    EXITB
                    cmp   eax,ecx
...
remote_kd_intr:
  ...
  CANDB  esp,<offset tcb_space+tcb_space_size>
        ...
        FI
        mov   eax,offset kdebug_timer_intr_handler+KR
  ELSE_
        mov   al,'-'
        kd____outchar
        sub   eax,eax
...
init_trace_buffer:
  ...
  IFNZ  [physical_kernel_info_page].kdebug_pages,0,long
...
set_performance_tracing:
  ...
  ELIFZ al,p6_family,long
        ...
        kd____disp <'x : uuee0/uuee1',13,10>  
  FI 
  ...
  ELIFZ ah,p6_family,long
        ...
        FI
        IFZ   al,'2'
              mov   bl,P6_L2_rw_miss 
              mov   cl,P6_L2_d_wback
        FI
        IFZ   al,'i'
              mov   bl,P6_ex_miss
              mov   cl,P6_ex_tlb
