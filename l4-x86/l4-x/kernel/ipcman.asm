include l4pre.inc 
  ...
  Copyright IBM+UKA, L4.IPCmvs, 07,09,00, 9182, K 
...
intr_while_dispatching:
  ...
  cpu___cycles ebx        ; ebx = dispatcher_tcb
...
;;;;;;;;;;;;  switch_space
  ...
  IFNZ  ebp,ds:[gdt+linear_space/8*8+4]
...
XHEAD send_only
  ...
  if    prio_sensitive_donation_on_send
  
        mov   cl,[ebp+prio]
        mov   ch,[ebx+prio]
        cmp   cl,ch
        jb    send_without_donate
...
XHEAD ipc_strings
  ...
  DO
        ...
        CANDB ebx,virtual_space_size,long
              ...
              lno___task  edx,ebp                       ;;
              mov   edx,ds:[edx*8+task_proot].proot_ptr ;;
              IFNZ  edx,ds:[cpu_cr3]                    ;;
                    mov   edx,edi
                    and   edx,-MB4
                    sub   edi,edx
...
XHEAD pending_or_auto_propagating
  ...
  mov   edi,[ebx+myself]
  DO
        mov   dl,[ebp+coarse_state]
        add   ebp,sizeof tcb
        test  dl,auto_propagating
        EXITZ
