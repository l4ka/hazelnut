include l4pre.inc 
  ...
  Copyright IBM+UKA, L4.IPCMAN, 15,04,00, 9580, K 
...
delete_from_sndq_ss macro

 mov eax,ss:[ebp+sndq_llink].succ
 mov ecx,ss:[ebp+sndq_llink].pred

 mov ss:[eax].pred,ecx
 mov ss:[ecx].succ,eax

 IFZ eax,ecx
     and ss:[eax+list_state-offset sndq_root],NOT is_polled
 FI
...
ipc_sc:
  ...
  mov   ds,[ebp+thread_seg]
  mov   es,[ebp+thread_seg]
...
ipc_sysenter:          
  ...
  mov   ss:[ebp+waiting_for],esi
  mov   ss:[ebp+mword2],edi
  mov   ss:[ebp+timeouts],ecx
  mov   ss:[ebp+rcv_descriptor],ebx
...
ipc_restart:
  ...
  jae   receive_only
        ...
        shr   edx,task_no-log2_tasks-2                  ;
                                                        ;
        mov   edx,ss:[edx+ebp+redirection_table]           ;
                                                        ;
  ...
  mov   dl,ss:[ebx+ebp+fine_state]
  
  and   dl,nwait+nclos
  IFLE                                      ; Greater : nwait=SF=OV=0 and ZF=0
        IFZ   
              mov   esi,ss:[ebx+ebp+waiting_for]
              mov   edi,ss:[ebp+myself]   
              cmp   edi,esi      
        FI      
  ...
  mov   edx,ss:[ebp+rcv_descriptor]
  mov   ch,running
  
  mov   ss:[ebx+ebp+fine_state],ch
  ...
  ELSE__
        ...
        and   edx,nclos   
        mov   edi,dword ptr ss:[ebp+fine_state]
        shr   edi,zpolled+16
        test  edi,edx
...
;;;;; switch_thread ipc,edi
  ...
  lea   esi,[edi+ebp+sizeof tcb]
  mov   ss:[ebp+thread_esp],esp
  
  mov   ss:[cpu_esp0],esi
  mov   esp,ss:[edi+ebp+thread_esp]
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  mov   ecx,ss:[edi+ebp+thread_seg]
  mov   edi,ss:[ebp+mword2]  
...
;;;;;;;;;;;;  switch_space
  ...
  IFZ   ecx,flat_seg
        ...
        mov   ebp,esp
        and   ebp,-sizeof tcb
        mov   ecx,ss:[ebp+thread_proot]

        IFNZ  dword ptr ss:[cpu_cr3],ecx
           
              mov   dword ptr ss:[cpu_cr3],ecx
              mov   dword ptr ss:[tlb_invalidated],ecx
              mov   cr3,ecx
        FI
  FI
...
XHEAD propagate_or_redirected_post
 ...
 ;kernel_ds
  ...
  mov   ecx,ss:[ebp+waiting_for]
  test  al,deceit
 
  mov   esi,ss:[ebp+myself]
  xret  z,long

  mov   esi,ss:[ebp+virtual_sender]
  xret  ,long
...
XHEAD enter_wakeup_for_receive_timeout
 ...
 ;kernel_ds
  ...
  mov   ss:[esp+ipc_eax],offset receive_timeout_ret+KR
  ...
  IFNZ
        ...
        add   edi,ss:[system_clock_low]
        mov   ss:[ebp+wakeup_low],edi
...
XHEAD ipc_long_ss
  ...
  CANDNZ ,,long
        ...
        xc    ns,mwords_through_com_space,long
...
XHEAD pending_or_auto_propagating
 ...
 ;kernel_ds
  ...
  mov   edi,ss:[ebx+ebp+myself]
  DO
        test  ss:[ebx+ebp+coarse_state],auto_propagating
        EXITZ
        ...
        test  dl,nclos
        IFNZ
              cmp   ss:[ebx+ebp+waiting_for],edi
              REPEATNZ
        FI      
        ...
        test  al,deceit
        IFNZ
        mov   esi,ss:[ebp+propagatee_tcb]
        CANDNZ esi,0
              mov   ss:[esi+waiting_for],edi
        ELSE_
              mov   ss:[ebp+waiting_for],edi
        FI            
...
ret_from_poll:
  ...
  mov   ebp,esp
  and   ebp,-sizeof tcb
  mov   ebx,ss:[ebp+com_partner]
  sub   ebx,ebp

  mov   eax,ss:[esp+ipc_eax]
  mov   ecx,ss:[ebp+timeouts]
  mov   esi,ss:[ebp+waiting_for]

  IFZ   ss:[ebp+fine_state],locked_running

        mov   ss:[ebp+fine_state],running
        and   ss:[ebx+ebp+fine_state],nclos
        or    ss:[ebx+ebp+fine_state],closed_wait+nwake

        jmp   ipc_restart

  FI

  test  ss:[ebp+fine_state],npoll
  IFZ
        delete_from_sndq_ss
        mov   [ebp+fine_state],running
  FI
