include l4pre.inc 
  ...
  Copyright IBM+UKA, L4.PAGCTR, 21,09,99, 117
...
enable_paging_mode:
  ...
  bt    ds:[pre_paging_cpu_feature_flags],global_pages_bit
  IFC        
        mov   eax,cr4
        bts   eax,cr4_enable_global_pages_bit
        mov   cr4,eax
  FI
...
map_page_initially:
  ...
  mov   al,cl
  or    al,page_user_permit                 ; due to sysexit !!!
  IFAE  esi,begin_of_shared_space
  CANDB esi,end_of_shared_space
        or    eax,global_page
  FI
  mov   [ebx],eax
...
map_fresh_ptab:
  ...
  ELSE_
        ...
        mov   al,page_present+page_write_permit+page_user_permit    ; user_permit due to sysexit !!!
...
map_system_shared_page:
  ...
  or    edx,page_present+page_write_permit+page_user_permit+global_page
  mov   [ecx],edx                               ; user_permit due to sysexit !!!
